...
func _raytrace_end(start: Vector2, end: Vector2):
    var q = PhysicsRayQueryParameters2D.new()
    q.from = start
    q.to = end + (end - start).normalized()*10000
    q.collision_mask = 0b1 # "Collision" only
    var result = get_world_2d().direct_space_state.intersect_ray(q)
    return result


func set_start_end(start: Vector2, end: Vector2):
    print(end)
    var raytrace_result = _raytrace_end(start, end)
